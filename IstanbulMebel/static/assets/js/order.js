...
function init() {
    console.log('🚀 Master JavaScript loaded - FIXED SCROLL OFFSET');
...
function normalizeItem(item, forCart = true) {
    ...
    return { id, title, image, price: price, unit_price: price, quantity, total_price: totalPrice };
}
...
function createDropdownListItem(item, forCart = true) {
    ...
    let priceText = '';
    if (forCart) { priceText = `$${priceFormatted} x ${quantity} = $${totalFormatted}`; }
    else { priceText = `$${priceFormatted}`; }
    li.innerHTML = `<img src="${item.image}" alt="${escapeHtml(item.title)}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 5px;" onerror="this.src='https://via.placeholder.com/40?text=No+Image'"><div class="item-details"><div class="item-title">${escapeHtml(item.title) || 'Məhsul'}</div><div class="item-price" style="color: #fff; font-size: 12px;">${priceText}</div></div>`;
    return li;
}
...
function insertOptimisticItemToDropdown(rawItem, dropdownId = 'cartDropdown') {
    ...
    if (!container) {
        ...
        if (firstLi) firstLi.before(container);
    ...
    if (normalized.id) {
        ...
        if (existing) {
            ...
            if (priceEl) {
                ...
                if (dropdownId.includes('cart')) priceEl.textContent = `$${price.toFixed(2)} x ${newQty} = $${total.toFixed(2)}`;
                const headerCount = dropdown.querySelector('.header-count');
                if (headerCount) { const curr = parseInt(headerCount.textContent) || 0; headerCount.textContent = `${curr + normalized.quantity} məhsul`; }
                updateCartTotal();
            }
            return;
        }
    ...
    const headerCount = dropdown.querySelector('.header-count');
    if (headerCount) { const curr = parseInt(headerCount.textContent) || 0; headerCount.textContent = `${curr + normalized.quantity} məhsul`; }
    if (dropdownId.includes('cart')) updateCartTotal();
}
    ...
    items.forEach(item => {
        ...
        if (priceEl) {
            ...
            if (totalMatch) total += parseFloat(totalMatch[1]);
                ...
                if (priceMatch && qtyMatch) total += parseFloat(priceMatch[1]) * parseInt(qtyMatch[1]);
            }
        }
    });
...
function removeOptimisticItemsFromDropdown(productId, dropdownId = 'cartDropdown') {
    ...
    if (dropdownId === 'cartDropdown') updateCartTotal();
...
function initDropdowns() {
    ...
    const cartDropdown = document.getElementById('cartDropdown');
    if (!wishlistBtn || !cartBtn || !wishlistDropdown || !cartDropdown) { console.warn('Dropdown elements not found'); return; }
    wishlistBtn.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); cartDropdown.classList.remove('show'); wishlistDropdown.classList.toggle('show'); if (wishlistDropdown.classList.contains('show')) refreshWishlistDropdown(); });
    cartBtn.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); wishlistDropdown.classList.remove('show'); cartDropdown.classList.toggle('show'); if (cartDropdown.classList.contains('show')) refreshCartDropdown(); });
    document.addEventListener('click', function(e) { if (!wishlistBtn.contains(e.target) && !wishlistDropdown.contains(e.target)) wishlistDropdown.classList.remove('show'); if (!cartBtn.contains(e.target) && !cartDropdown.contains(e.target)) cartDropdown.classList.remove('show'); });
    document.addEventListener('keydown', function(e) { if (e.key === 'Escape') { wishlistDropdown.classList.remove('show'); cartDropdown.classList.remove('show'); } });
...
function initProductQuantity() {
    ...
    window.decreaseQuantity = function() { let currentValue = parseInt(quantityInput.value) || minValue; if (currentValue > minValue) { quantityInput.value = currentValue - 1; hiddenInput.value = quantityInput.value; updateTotalPrice(quantityInput.value); } };
    window.increaseQuantity = function() { let currentValue = parseInt(quantityInput.value) || minValue; if (currentValue < maxValue) { quantityInput.value = currentValue + 1; hiddenInput.value = quantityInput.value; updateTotalPrice(quantityInput.value); } };
...
function extractProductDataFromForm(form) {
    ...
    if (!title) { const productCard = form.closest('.product-card, .product-item, .portfolio-item, [class*="product"]'); if (productCard) title = productCard.querySelector('.product-title, h3, h4, .title')?.textContent?.trim() || ''; }
    ...
    if (!image) { const productCard = form.closest('.product-card, .product-item, .portfolio-item, [class*="product"]'); if (productCard) { const img = productCard.querySelector('img'); if (img) image = img.src; } }
    if (!image) { const mainImg = document.querySelector('.product-image img, .main-image img, .product-detail img, .product-main-image img'); if (mainImg) image = mainImg.src; }
    let priceRaw = '';
    ...
    if (!priceRaw) { const productCard = form.closest('.product-card, .product-item, .portfolio-item, [class*="product"]'); if (productCard) { priceRaw = productCard.querySelector('.special-price-value, .current-price, .product-price, .price')?.textContent || ''; if (!priceRaw) priceRaw = productCard.querySelector('[class*="price"]')?.textContent || ''; } }
    if (!priceRaw) { priceRaw = document.querySelector('.special-price-value, .current-price, .product-price')?.textContent || ''; if (!priceRaw) priceRaw = document.querySelector('.price-section .current-price, .price-section .special-price')?.textContent || ''; }
    ...
    return { id: productId, title: title, image: image, price: price, quantity: quantity };
}
...
function setupAddToCartForm(form) {
    ...
    newForm.addEventListener('submit', async function(e) {
        ...
        const optimisticItem = { id: productData.id, title: productData.title, image: productData.image, price: productData.price, quantity: quantity };
        try { insertOptimisticItemToDropdown(optimisticItem, 'cartDropdown'); const currCount = parseInt(document.querySelector('.cart-count, #cartBtn .dropdown-badge')?.textContent) || 0; updateCartCount(currCount + quantity); } catch (err) { console.warn('Optimistic add-to-cart failed', err); }
        try {
            const response = await fetch(this.action, { method: 'POST', body: formData, headers: { 'X-Requested-With': 'XMLHttpRequest', 'X-CSRFToken': getCsrfToken() } });
            ...
            if (data?.success) {
                ...
                if (btn) { btn.innerHTML = '<i class="fas fa-check"></i>'; btn.style.backgroundColor = '#28a745'; setTimeout(() => { btn.innerHTML = originalHtml; btn.style.backgroundColor = ''; btn.disabled = false; }, 1400); }
            } else { removeOptimisticItemsFromDropdown(productId, 'cartDropdown'); updateCartCount(data?.cart_count || 0); showNotification(data?.message || '❌ Xəta baş verdi!', 'error'); if (btn) { btn.innerHTML = originalHtml; btn.disabled = false; } }
        } catch (error) { console.error('Fetch error:', error); removeOptimisticItemsFromDropdown(productId, 'cartDropdown'); showNotification('❌ Xəta baş verdi!', 'error'); if (btn) { btn.innerHTML = originalHtml; btn.disabled = false; } }
    });
}
...
function setupWishlistForm(form) {
    ...
    newForm.addEventListener('submit', async function(e) {
        ...
        const formData = new FormData(this);
        const productData = extractProductDataFromForm(this);
        const optimisticItem = { id: productData.id, title: productData.title, image: productData.image, price: productData.price, quantity: 1 };
        try { insertOptimisticItemToDropdown(optimisticItem, 'wishlistDropdown'); const currCount = parseInt(document.querySelector('.wishlist-count, #wishlistBtn .dropdown-badge')?.textContent) || 0; updateWishlistCount(currCount + 1); } catch (err) { console.warn('Optimistic add-to-wishlist failed', err); }
        try {
            const response = await fetch(this.action, { method: 'POST', body: formData, headers: { 'X-Requested-With': 'XMLHttpRequest', 'X-CSRFToken': getCsrfToken() } });
            const data = await response.json();
            if (data?.success) {
                const action = data.action || 'added';
                if (action === 'added') { showNotification(data.message || '❤️ Məhsul wishlist-ə əlavə edildi!', 'success'); if (btn) { btn.innerHTML = '<i class="fas fa-heart" style="color:#ff6b6b"></i>'; btn.style.backgroundColor = '#fee9e9'; } }
                else { showNotification(data.message || '💔 Məhsul wishlist-dən silindi!', 'info'); if (btn) { btn.innerHTML = '<i class="far fa-heart"></i>'; btn.style.backgroundColor = ''; } removeOptimisticItemsFromDropdown(productId, 'wishlistDropdown'); }
                updateWishlistCount(data.wishlist_count || 0);
                await refreshWishlistDropdown();
                await refreshCartDropdown();
                setTimeout(() => { if (btn && action !== 'added') btn.disabled = false; }, 800);
            } else { removeOptimisticItemsFromDropdown(productId, 'wishlistDropdown'); updateWishlistCount(data?.wishlist_count || 0); showNotification(data?.message || '❌ Xəta baş verdi!', 'error'); if (btn) { btn.innerHTML = originalHtml; btn.disabled = false; } }
        } catch (error) { console.error('Fetch error:', error); removeOptimisticItemsFromDropdown(productId, 'wishlistDropdown'); showNotification('❌ Xəta baş verdi!', 'error'); if (btn) { btn.innerHTML = originalHtml; btn.disabled = false; } }
    });
}
...
async function refreshCartDropdown() {
    try {
        ...
        let res = null;
        for (const ep of endpoints) { try { res = await fetch(ep, { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest', 'Cache-Control': 'no-cache' } }); if (res.ok) break; } catch (e) { } }
...
async function processCartData(data, cartDropdown) {
    ...
    if (!itemsContainer) {
        ...
        if (firstLi) firstLi.before(itemsContainer);
        else cartDropdown.appendChild(itemsContainer);
    }
    itemsContainer.innerHTML = '';
    if (data.items && data.items.length > 0) {
        data.items.forEach(raw => { const item = normalizeItem(raw, true); const li = createDropdownListItem(item, true); if (item.id) li.dataset.productId = item.id; itemsContainer.appendChild(li); });
        const totalAmount = cartDropdown.querySelector('.total-amount');
        if (totalAmount) totalAmount.textContent = `$${parseFloat(data.cart_total ?? 0).toFixed(2)}`;
    } else { itemsContainer.innerHTML = `<li class="dropdown-item empty-placeholder" style="justify-content:center;color:#999;text-align:center;padding:20px;"><i class="fas fa-shopping-cart" style="font-size:24px;margin-bottom:8px;display:block;"></i><span>Səbət boşdur</span></li>`; const totalAmount = cartDropdown.querySelector('.total-amount'); if (totalAmount) totalAmount.textContent = '$0.00'; }
}
...
async function processWishlistData(data, wishlistDropdown) {
    ...
    if (!itemsContainer) {
        ...
        if (firstLi) firstLi.before(itemsContainer);
        else wishlistDropdown.appendChild(itemsContainer);
    }
    itemsContainer.innerHTML = '';
    if (data.items && data.items.length > 0) { data.items.forEach(raw => { const item = normalizeItem(raw, false); const li = createDropdownListItem(item, false); if (item.id) li.dataset.productId = item.id; itemsContainer.appendChild(li); }); }
    else { itemsContainer.innerHTML = `<li class="dropdown-item empty-placeholder" style="justify-content:center;color:#999;text-align:center;padding:20px;"><i class="far fa-heart" style="font-size:24px;margin-bottom:8px;display:block;"></i><span>Wishlist boşdur</span></li>`; }
}

// Update counts
function updateCartCount(count) {
    const selectors = ['.cart-count', '.cart-badge', '[class*="cart-count"]', '#cartBtn .dropdown-badge'];
    selectors.forEach(selector => { $$(selector).forEach(el => { if (el) { el.textContent = count; el.classList.add('pulse'); setTimeout(() => el.classList.remove('pulse'), 500); if (count == 0) el.style.opacity = '0.6'; else el.style.opacity = '1'; } }); });
}
...
function showNotification(message, type = 'success') {
    ...
    const config = { success: { icon: 'fa-check-circle', bg: '#28a745', color: '#fff' }, error: { icon: 'fa-exclamation-circle', bg: '#dc3545', color: '#fff' }, info: { icon: 'fa-info-circle', bg: '#17a2b8', color: '#fff' }, warning: { icon: 'fa-exclamation-triangle', bg: '#ffc107', color: '#000' } };
...
function initCartQuantity() {
    ...
    $$('.cart-row').forEach(row => {
        ...
        if (unitPriceEl) unitPrice = formatPrice(unitPriceEl.textContent);
        else { const priceEl = row.querySelector('.price, .unit-price, .product-price'); if (priceEl) unitPrice = formatPrice(priceEl.textContent); }
        ...
        if (qtyInput && unitPrice > 0) { const qty = parseInt(qtyInput.value) || 1; const newTotal = unitPrice * qty; const totalEl = document.getElementById(`totalPrice-${itemId}`); if (totalEl) totalEl.textContent = '$' + newTotal.toFixed(2); }
    });
    updateSummaryFromUI();
    const updateBtn = document.getElementById('updateCartBtn');
    if (updateBtn) { const newBtn = updateBtn.cloneNode(true); updateBtn.parentNode.replaceChild(newBtn, updateBtn); newBtn.addEventListener('click', function(e) { e.preventDefault(); submitCartUpdates(); }); }
}
...
function updatePriceForItem(itemId, quantity) {
    ...
    if (totalEl) { totalEl.textContent = '$' + newTotal.toFixed(2); totalEl.dataset.total = newTotal.toFixed(2); }
    updateSummaryFromUI();
}

function updateSummaryFromUI() {
    let subtotal = 0;
    $$('.cart-row').forEach(row => { const itemId = row.dataset.itemId; const totalEl = document.getElementById(`totalPrice-${itemId}`); if (totalEl) { const total = parseFloat((totalEl.textContent || '').replace(/[^0-9.]/g, '')) || 0; subtotal += total; } });
    const shipping = 0;
    ...
    if (itemCountEl) { const count = $$('.cart-row').length; itemCountEl.textContent = `${count} məhsul`; }
}

function submitCartUpdates() {
    const changedItems = [];
    $$('.cart-row').forEach(row => { const itemId = row.dataset.itemId; const currentQty = document.getElementById(`current-quantity-${itemId}`)?.value; const originalQty = window.originalQuantities[itemId]; if (currentQty && originalQty && currentQty !== originalQty) changedItems.push({ id: itemId, quantity: currentQty }); });
    ...
    if (csrfToken) { const csrfInput = document.createElement('input'); csrfInput.type = 'hidden'; csrfInput.name = 'csrfmiddlewaretoken'; csrfInput.value = csrfToken; form.appendChild(csrfInput); }
    changedItems.forEach(item => { const input = document.createElement('input'); input.type = 'hidden'; input.name = `quantity_${item.id}`; input.value = item.quantity; form.appendChild(input); });
    const countInput = document.createElement('input'); countInput.type = 'hidden'; countInput.name = 'changed_count'; countInput.value = changedItems.length; form.appendChild(countInput);
...
function initCartSearch() {
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        rows.forEach(row => {
            ...
            if (name.includes(term) || sku.includes(term) || man.includes(term)) { row.style.display = ''; visible++; row.style.backgroundColor = '#fff9e6'; setTimeout(() => row.style.backgroundColor = '', 500); }
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            ...
            rows.forEach(row => {
                const name = (row.querySelector('.product-name')?.textContent || '').toLowerCase();
                const sku = (row.querySelector('.product-sku')?.textContent || '').toLowerCase();
                if (name.includes(term) || sku.includes(term)) { row.style.display = ''; visible++; row.style.backgroundColor = '#fff9e6'; setTimeout(() => row.style.backgroundColor = '', 500); }
                else row.style.display = 'none';
            });
...
function initProductSearch() {
    ...
    function performProductSearch() {
        ...
        cards.forEach(card => {
            ...
            if ((title + ' ' + category + ' ' + desc).includes(term)) { card.style.display = ''; visible++; card.style.boxShadow = '0 0 0 2px #ff6b6b'; card.style.transform = 'scale(1.02)'; setTimeout(() => { card.style.boxShadow = ''; card.style.transform = ''; }, 500); }
            else card.style.display = 'none';
        });
        if (resultMsg) {
            if (visible === 0) { resultMsg.innerHTML = `<div style="padding:15px;background:#f8d7da;color:#721c24;border-radius:8px;display:flex;justify-content:space-between;align-items:center"><span><i class="fas fa-exclamation-circle"></i> No products found matching "<strong>${escapeHtml(term)}</strong>"</span><button class="clear-search-btn" style="background:none;border:1px solid #721c24;color:#721c24;padding:5px 15px;border-radius:30px;cursor:pointer">Clear</button></div>`; resultMsg.style.display = 'block'; const clearSearchBtn = resultMsg.querySelector('.clear-search-btn'); if (clearSearchBtn) clearSearchBtn.addEventListener('click', clearProductSearch); }
            else { resultMsg.innerHTML = `<div style="padding:15px;background:#d4edda;color:#155724;border-radius:8px;display:flex;justify-content:space-between;align-items:center"><span><i class="fas fa-check-circle"></i> Found <strong>${visible}</strong> product(s)</span><button class="clear-search-btn" style="background:none;border:1px solid #155724;color:#155724;padding:5px 15px;border-radius:30px;cursor:pointer">Clear</button></div>`; resultMsg.style.display = 'block'; const clearSearchBtn = resultMsg.querySelector('.clear-search-btn'); if (clearSearchBtn) clearSearchBtn.addEventListener('click', clearProductSearch); }
        }
    }
}

function updateSearchResultUI(element, term, visible, type) {
    if (visible === 0) { element.innerHTML = `<div style="padding:15px;background:#f8d7da;color:#721c24;border-radius:8px;display:flex;justify-content:space-between;align-items:center;width:100%"><span><i class="fas fa-exclamation-circle"></i> No products found matching "<strong>${escapeHtml(term)}</strong>"</span><button class="clear-search-btn" style="background:none;border:1px solid #721c24;color:#721c24;padding:5px 15px;border-radius:30px;cursor:pointer">Clear</button></div>`; }
    ...
    if (clearBtn) { clearBtn.addEventListener('click', () => { const input = type === 'cart' ? document.getElementById('cartSearch') : document.getElementById('wishlistSearch'); if (input) { input.value = ''; input.focus(); input.dispatchEvent(new Event('keyup')); } element.style.display = 'none'; }); }
}

window.clearProductSearch = function() { const input = document.getElementById('productSearch'); const clearBtn = document.getElementById('clearProductSearch'); const resultMsg = document.getElementById('productSearchResult'); if (input) { input.value = ''; input.focus(); document.querySelectorAll('.product-card, .portfolio-item, [class*="col-"]').forEach(c => c.style.display = ''); } if (clearBtn) clearBtn.style.display = 'none'; if (resultMsg) resultMsg.style.display = 'none'; };
window.clearWishlistSearch = function() { const input = document.getElementById('wishlistSearch'); if (input) { input.value = ''; input.focus(); input.dispatchEvent(new Event('keyup')); } const msg = document.getElementById('wishlistSearchResult'); if (msg) msg.style.display = 'none'; };

// Checkout - FIXED scroll to top after order
...
function initPriceSlider() {
    ...
    function updateThumbs() { const leftPercent = ((sliderLeft.value - min)/(max-min))*100; const rightPercent = ((sliderRight.value - min)/(max-min))*100; leftThumb.style.left = leftPercent + '%'; rightThumb.style.left = rightPercent + '%'; progressBar.style.left = leftPercent + '%'; progressBar.style.width = (rightPercent - leftPercent) + '%'; }
    function updateInputs() { minInput.value = sliderLeft.value; maxInput.value = sliderRight.value; if (hiddenMin) hiddenMin.value = sliderLeft.value; if (hiddenMax) hiddenMax.value = sliderRight.value; }
    updateThumbs();
    ...
    function setupDragThumb(thumb, isLeft) {
        ...
        thumb.addEventListener('mousedown', function(e){ e.preventDefault(); isDragging = true;
            function onMouseMove(e) { if (!isDragging) return; const rect = sliderLeft.parentElement.getBoundingClientRect(); let x = e.clientX - rect.left; let percent = Math.max(0, Math.min(100, (x / rect.width) * 100)); let value = Math.round((percent / 100) * max); if (isLeft) { if (value < parseInt(sliderRight.value)) { sliderLeft.value = value; sliderLeft.dispatchEvent(new Event('input')); } } else { if (value > parseInt(sliderLeft.value)) { sliderRight.value = value; sliderRight.dispatchEvent(new Event('input')); } } }
