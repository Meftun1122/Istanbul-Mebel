...
// ------------------------
// Utilities for data normalizing (DÜZƏLDİLDİ)
// ------------------------
function formatPrice(value) {
    if (value === undefined || value === null) return 0;
    if (typeof value === 'string') {
        // Əvvəlcə bütün boşluqları təmizlə
        let cleaned = value.trim();
        
        // Əgər mətn "AZN" və ya "₺" kimi valyuta işarəsi varsa, onu sil
        cleaned = cleaned.replace(/[AZN₺$€£]/g, '');
        
        // Vergülü nöqtəyə çevir (ancaq sonuncu vergülü)
        if (cleaned.includes(',')) {
        ...
        // Yalnız rəqəmlər və nöqtə qalsın
        cleaned = cleaned.replace(/[^0-9.]/g, '');
        
        // Birdən çox nöqtə varsa, yalnız birincisini saxla
...
// ------------------------
// Cart quantity control (DÜZƏLDİLDİ - Qiymət düzgün hesablanır)
// ------------------------
function initCartQuantity() {
    ...
    $$('.cart-row').forEach(row => {
        ...
        if (unitPriceEl) {
            unitPrice = formatPrice(unitPriceEl.textContent);
        } else {
            const priceEl = row.querySelector('.price, .unit-price, .product-price');
            if (priceEl) {
                unitPrice = formatPrice(priceEl.textContent);
            }
        }
...
function updatePriceForItem(itemId, quantity) {
    ...
    if (unitPrice === undefined || unitPrice === 0) {
        console.warn(`Unit price not found for item ${itemId}`);
        return;
    }
...
function updateSummaryFromUI() {
    ...
    if (itemCountEl) {
        ...
        itemCountEl.textContent = `${count} məhsul`;
    }
